...
st.set_page_config(page_title="Streamlit Data Explorer ", layout="wide", page_icon="📊")
...
# Set funky header
st.title('VIZGENIUS Interactive Data Explorer')
...
if uploaded_file1 is not None and uploaded_file2 is not None:
    ...
    st.sidebar.markdown('Your First Dataset')
    st.sidebar.write(df1.head())

    # Display the second dataset
    st.sidebar.markdown('Your Second Dataset')
    st.sidebar.write(df2.head())

    # Basic data statistics for the first dataset
    st.sidebar.markdown('The Summary Statistics for First Dataset')
